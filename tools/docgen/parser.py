...
from .models import (
    Field, Method, Param, ParsedClass, ParsedHeader,
    ParsedNamespaceHelper, Struct,
)
...
def _parse_method(normalized: str, raw: str, class_name: str) -> Optional[Method]:
    ...
    for kw in ['virtual', 'override', 'explicit', 'static', 'inline',
...
def _parse_namespace_helper_entity(namespace: str, body: str) -> ParsedNamespaceHelper:
    ...
    body_clean = re.sub(r'\{[^{}]*\}', '', body_clean)  # remove any remaining inline bodies
    ...
    return ParsedNamespaceHelper(namespace=namespace, structs=structs, functions=functions)
